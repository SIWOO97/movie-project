...
void search_print(LINK_M *search_movie){
  LINK_W *actor;
  ...
  if(word_l == word_q){
    while(search_movie -> next_movie != movie -> tail){
      ...
      if(strcmp(string, search_movie->title) == 0)
        search_print(search_movie);
        else if(strcmp(string, search_movie -> genre) == 0)
          search_print(search_movie);
        else if(strcmp(string, search_movie -> director -> data) == 0)
          search_print(search_movie);
        else if(atoi(string) == search_movie -> year)
          search_print(search_movie);
        else if(atoi(string) == search_movie -> time)
          search_print(search_movie);
        else{
          actor = search_movie -> actor;
    ...
    while(search_movie -> next_movie != movie -> tail){
      ...
      else if(strlen(search_movie -> genre) == word_l){
        word_sq = 0;
        for(int i = 0; i < word_l; i++){
      ...
      else if(strlen(search_movie -> director -> data) == word_l){
        word_sq = 0;
        for(int i = 0; i < word_l; i++){
...
void test(ALL_M *movie){
  LINK_M * test = movie -> head;
  printf("%s", test -> next_movie-> actor -> data);
}
