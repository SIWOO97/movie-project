...
int update_movie(LINK_M *movie,int serial, char *option){
  printf("%d", serial);
  printf("%s\n", option);
  return 1;
...
void search(LINK_M *movie, char *input_data, char *search_string){
  LINK_M *search_movie, *search_con_movie;
  search_movie = (LINK_M*)malloc(sizeof(LINK_M));
  search_con_movie = (LINK_M*)malloc(sizeof(LINK_M));
  search_con_movie = movie;
  search_movie = movie -> next_movie;
  if(strcmp(input_data, "m") == 0){
    while(search_con_movie -> next_movie != NULL){
      if(strcmp(search_string, search_movie -> title) == 0){
      ...
      else if(strcmp(search_string, search_movie -> genre) == 0){
        printf("%d:%s:%s:%s:%d:%d:",search_movie -> serial_num, search_movie -> title, search_movie -> genre, search_movie -> director -> data, search_movie -> year, search_movie -> time);
        while(search_movie -> actor ->  next != NULL){
          printf("%s,", search_movie -> actor -> data);
          search_movie -> actor = search_movie -> actor -> next;
      ...
      else if(strcmp(search_string, search_movie -> director -> data) == 0){
        printf("%d:%s:%s:%s:%d:%d:",search_movie -> serial_num, search_movie -> title, search_movie -> genre, search_movie -> director -> data, search_movie -> year, search_movie -> time);
        while(search_movie -> actor ->  next != NULL){
          printf("%s,", search_movie -> actor -> data);
          search_movie -> actor = search_movie -> actor -> next;
      ...
      else if(atoi(search_string) == search_movie -> year){
        printf("%d:%s:%s:%s:%d:%d:",search_movie -> serial_num, search_movie -> title, search_movie -> genre, search_movie -> director -> data, search_movie -> year, search_movie -> time);
        while(search_movie -> actor ->  next != NULL){
          printf("%s,", search_movie -> actor -> data);
          search_movie -> actor = search_movie -> actor -> next;
      ...
      else if(atoi(search_string) == search_movie -> time){
        printf("%d:%s:%s:%s:%d:%d:",search_movie -> serial_num, search_movie -> title, search_movie -> genre, search_movie -> director -> data, search_movie -> year, search_movie -> time);
        while(search_movie -> actor ->  next != NULL){
          printf("%s,", search_movie -> actor -> data);
          search_movie -> actor = search_movie -> actor -> next;
      ...
      search_con_movie = search_con_movie -> next_movie;
...
void commands(LINK_D *director,LINK_A *actor,LINK_M *movie){
  ...
  char *command,*input_data, *search_string;//문자열을 입력받을 char형 포인터 배열을 선언하였음
  input_data = (char*)calloc(10, sizeof(char));
  search_string = (char*)calloc(10, sizeof(char));
  ...
  if(strcmp(command, "add") == 0){//파일에 새로운 내용을 더함
    ...
    getchar();
    scanf("%[^\n]", search_string);
    search(movie, input_data, search_string);
