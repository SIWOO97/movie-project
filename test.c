...
void add_actor(ALL_A *actor){
	...
	printf("name >");
	scanf("%[^\n]%*c",input_data);
	save_actor -> name = (char *)calloc(strlen(input_data)+1, sizeof(char));
...
int delete_actor(ALL_A *actor,int serial){
	FILE *act_d = fopen("actor_log", "a");
	LINK_A *pre_actor = (LINK_A *)malloc(sizeof(LINK_A));
	LINK_A *del_actor = (LINK_A *)malloc(sizeof(LINK_A));
...
int add_actor_list(ALL_A *actor,FILE *act_l){
	char read_text;
	int read_num;
	LINK_A *link_actor = (LINK_A *)malloc(sizeof(LINK_A));
	LINK_A *new_actor = (LINK_A *)malloc(sizeof(LINK_A));
	char *read_data = (char *)malloc(sizeof(char)*1000);
	new_actor -> title = (LINK_W *)malloc(sizeof(LINK_W));
	LINK_W *tmp = new_actor ->title;
...
int copy_movie_list(){
  ...
  int copy_actor_list(){
    ...
    int copy_director_list(){
      ...
      m2 = t -> tm_min;
      s = t -> tm_sec;
      sprintf(filename, "director_list%d%d%d%d%d%d", t -> tm_year+1900, t -> tm_mon, t -> tm_mday, t -> tm_hour, t -> tm_min, t -> tm_sec);
      //파일열기
      f2 = fopen(filename, "w");
